...
class IncomeProcessor:
    ...
    def process_main_data(self):
        self.commission_data.columns = self.commission_data.columns.astype(str)
        self.retention_data.columns = self.retention_data.columns.astype(str)
        self.commission_data['상품군분류'] = self.commission_data['상품군분류'].fillna('상품군분류')
        self.retention_data['상품군분류'] = self.retention_data['상품군분류'].fillna('상품군분류')
        
        self.main_data['상품군분류'] = self.main_data['상품군분류'].fillna('상품군분류')
    ...
    def process(self):
        ...
        self.add_five_columns(['life_ins_data','nonlife_ins_data'])
        self.get_m_count(self.commission_data, ['보험사', '상품군분류'],['보험사','상품군분류'], '수익비용인식회차','수익비용인식회차')
        self.get_m_count(self.commission_data, ['보험사','상품군분류'],['보험사','상품군분류'], '환수율인식회차','환수율적용회차')
        self.main_data[['환수율적용회차'  ]] = self.main_data[['환수율적용회차'  ]].fillna(0)
        
        self.main_data['환수율'] = self.lookup_value(self.commission_data, ['보험사','상품군분류'], ['보험사','상품군분류'], '당기해당회차', [i for i in range(25)], '' )
        self.main_data['유지율'] = self.lookup_value(self.retention_data, ['보험사','상품군분류'], ['회사명','상품군분류'], '당기해당회차', [i for i in range(25)], '' )
        self.main_data[['환수율','유지율'  ]] = self.main_data[['환수율','유지율']].fillna(0)
        
        self.main_data['성과(당월)'] = self.sum_by_company_date(self.life_ins_data,['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '성과', 'tempo_col') + self.sum_by_company_date(self.nonlife_ins_data,['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '성과', 'tempo_col'  )
        self.main_data['계약관리(당월)'] = self.sum_by_company_date(self.life_ins_data,['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '계약관리', 'tempo_col')+self.sum_by_company_date(self.nonlife_ins_data,['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '계약관리', 'tempo_col')
        self.main_data['수금(당월)'] = self.sum_by_company_date(self.life_ins_data,['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '수금', 'tempo_col')+self.sum_by_company_date(self.nonlife_ins_data,['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '수금', 'tempo_col')
        self.main_data['운영(당월)'] = self.sum_by_company_date(self.life_ins_data,['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '운영', 'tempo_col')
        self.main_data['기타(당월)'] = self.sum_by_company_date(self.life_ins_data,['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '기타', 'tempo_col')
