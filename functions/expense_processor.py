...
class ExpenseProcessor:
    ...
    def process_main_df(self):
        self.main_df['업적월'] = self.main_df['보험사+업적월+상품군 key'].str.split('/', expand = True)[1]
    ...
    def process(self):
        ...
        #print(self.prev_month_df.columns)
        
        self.get_m_count(self.commission_df, ['보험사'],['회사명'], '비용인식회차','수익비용인식회차')
        self.main_df['환수율적용회차'] = 18
        self.main_df['환수율(성과수수료)'] = self.lookup_value(self.commission_df, '보험사', '회사명', '당기해당회차', [i for i in range(1,19)], '회차별환수율정보_신계약성과_' )
        self.main_df['환수율(유지성과수수료)'] = self.lookup_value(self.commission_df, '보험사', '회사명', '당기해당회차', [i for i in range(13,26)], '추가회차별환수율정보_신계약성과_' )
        self.main_df['유지율'] = self.lookup_value(self.retention_df, '보험사', '회사명', '당기해당회차', [i for i in range(1,26)], '' )
        
        self.main_df['[지급수수료] 신계약성과(당월)'] = self.sum_by_company_date(self.security_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '[지급수수료] 신계약성과', 
                                                                'temporary_col')+self.sum_by_company_date(self.retirement_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], 
                                                                                                     '[지급수수료] 신계약성과', 'temporary_col')

        self.main_df['[지급수수료] 유지관리(당월)'] = self.sum_by_company_date(self.security_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '[지급수수료] 유지관리', 
                                                               'temporary_col')+self.sum_by_company_date(self.retirement_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'],
                                                                                                    '[지급수수료] 유지관리', 'temporary_col')

        self.main_df['[지급수수료] 유지성과(당월)'] = self.sum_by_company_date(self.security_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '[지급수수료] 유지성과', 
                                                               'temporary_col')+self.sum_by_company_date(self.retirement_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '[지급수수료] 유지성과', 
                                                                                                    'temporary_col')
        
        self.main_df['[지급수수료] 자동차(당월)'] = self.sum_by_company_date(self.security_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '[지급수수료] 자동차', 'temporary_col')
        
        self.main_df['[지급수수료] 일반(당월)'] = self.sum_by_company_date(self.security_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '[지급수수료] 일반', 'temporary_col')
        
        self.main_df['[지급수수료] 오버라이드성과(당월)'] = self.sum_by_company_date(self.override_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '[지급수수료] 성과', 'temporary_col')
        
        self.main_df['[지급수수료] 오버라이드육성(당월)'] = self.sum_by_company_date(self.override_df, ['보험사+업적월+상품군 key'], ['보험사+업적월+시책회차 key'], '[지급수수료] 육성', 'temporary_col')
                
        
        self.main_df['[지급수수료] 신계약성과(누적)'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                        ['보험사+업적월+상품군 key'],
                                                        '[지급수수료] 신계약성과(누적)', 
                                                        'temporary_col')+self.main_df['[지급수수료] 신계약성과(당월)']

        self.main_df['[지급수수료] 유지관리(누적)'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '[지급수수료] 유지관리(누적)', 
                                                                'temporary_col')+self.main_df['[지급수수료] 유지관리(당월)']
        
        self.main_df['[지급수수료] 유지성과(누적)'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '[지급수수료] 유지성과(누적)', 
                                                                'temporary_col')+self.main_df['[지급수수료] 유지성과(당월)']
        
        self.main_df['[지급수수료] 자동차(누적)'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '[지급수수료] 자동차(누적)', 
                                                                'temporary_col')+self.main_df['[지급수수료] 자동차(당월)']
        
        self.main_df['[지급수수료] 일반(누적)'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '[지급수수료] 일반(누적)', 
                                                                'temporary_col')+self.main_df['[지급수수료] 일반(당월)']
        
        self.main_df['[지급수수료] 오버라이드성과(누적)'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '[지급수수료] 오버라이드성과(누적)', 
                                                                'temporary_col')+self.main_df['[지급수수료] 오버라이드성과(당월)']
        
        self.main_df['[지급수수료] 오버라이드육성(누적)'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '[지급수수료] 오버라이드육성(누적)', 
                                                                'temporary_col')+self.main_df['[지급수수료] 오버라이드육성(당월)']
        
        self.main_df['기초선급비용'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '기말선급비용', 
                                                                'temporary_col')
        ...
        self.main_df['전월누적비용인식액'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '당월누적비용인식액', 
                                                                'temporary_col')
        ...
        self.main_df['기초환수자산'] = self.sum_by_company_date(self.prev_month_df, ['보험사+업적월+상품군 key'],
                                                                ['보험사+업적월+상품군 key'],
                                                                '기말환수자산', 
                                                                'temporary_col')
        ...
        self.main_df.loc[self.main_df['보험사+업적월+상품군 key'] == '이관계약', columns_to_modify] = 0
